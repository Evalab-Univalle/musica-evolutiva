...
function rateStars(spn) {
  ...
  if(itemrated_rtg == '' && spn.parentNode.parentNode.parentNode) itemrated_rtg = spn.parentNode.parentNode.parentNode.innerHTML;       // store the rating of rated item
    ...
    if(spn.parentNode.parentNode.parentNode) spn.parentNode.parentNode.parentNode.childNodes[0].innerHTML = '<i>Calificacion:</i><span>'+(i_sp+1)+'</span>';		// shows the choosed rating

    // Modify the length and background of the zone with vizible stars (different for Firefox)
    if (navigator.userAgent.indexOf("Firefox")!=-1) star_sp.setAttribute('style', 'width:'+((i_sp+1)*22)+'px; background:url("/musievol/ratingfiles/star2.png")');
    else {
      star_sp.style.width = ((i_sp+1)*22)+'px';
      star_sp.style.background = 'url("/musievol/ratingfiles/star2.png")';
    }
  }
}
...
function addRtgData(elm, totalrate, nrrates, renot) {
  ...
  var htmlrtg = '<div class="totalrate"><i>Calificacion:</i>0</div>'+
                '<div class="stars"><div class="star_n" style="width:0px;">&nbsp;</div>'+d_rtg+'</div>'
...
function rateIt(spn) {
  ...
  spn.parentNode.parentNode.parentNode.parentNode.innerHTML = '<i><b>Gracias por la colaboraci&oacute;n</b></i>';
  rtgAjax(elm, nota);
}
...
function rtgAjax(elm, ratev) {
  ...
  cerere_http.open("POST", '/musievol/ratingfiles/ratings.php', true);			// crate the request
